<template>
  <div><!--待审核商品-->
    <a-card :bordered="false">
      <OnlineSearchForm :user-list="data.goodsList" @search="search"></OnlineSearchForm>
    </a-card>
...
import OnlineSearchForm from '@/views/order/manage/components/OnlineSearchForm'
...
export default {
  ...
  methods: {
    ...
    getOrderList () {
      ...
      getAllOrders(requestParameters).then(response => {
        this.data.goodsList = response.data
        console.log('orderList', this.data.goodsList)
      }) */
      this.data.goodsList = [{ type: 0 }, { orderState: 0 }]
    }
    /* forceQuit (id) {
      const _this = this
